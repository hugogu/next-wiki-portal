**next-wiki** 是一个自托管、AI 辅助的个人知识库，面向运行多个 AI Agent 的个人。它为 owner 提供持久的工作空间，用来管理页面、文档、规则、非敏感配置、对话、来源证据、流程、记忆和精选知识。Agent 只能检索被允许看到的上下文，哪些内容进入共享空间或公开发布，由 owner 明确决定。

> **项目状态：** 积极开发中，处于早期开源发布阶段，接口与配置可能演进。

## 为什么是 next-wiki

多数 wiki 擅长存页面，多数 AI 助手擅长答问题。next-wiki 把两者连成一个**从知识到上下文的闭环**：
...
目标是让知识对人和 Agent 都有用，同时让所有权、证据、权限、修订与发布边界始终可见。Agent 检索到的上下文只是数据，不是权威：它不能授予权限、授权工具或执行命令。

## 亮点

- **真正的个人知识库。** 页面、文档、来源证据、决策和修订都在 Wiki 中保持可读、可编辑，AI 帮你沉淀和组织知识。
- **一个 owner，多个 Agent。** 产品模型是一组个人 Agent：每个 Agent 可以拥有自己的工作上下文，共享命名空间必须明确设置，不会意外混在一起。
- **知识不止对话。** 规则、非敏感配置、流程、情景记忆、来源证据和精选知识是不同类型的持久知识，而不是一堆没有区分的聊天记录。
- **用对话建设知识。** Wiki AI 对话基于关键字 + 语义混合检索并展示引用来源，可通过对话起草页面、重组目录树、翻译与策展。
